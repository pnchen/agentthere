...
/** Resample mono PCM16 to the requested sample rate. */
export function resamplePcm(pcm16, fromRate, toRate) {
    if (fromRate === toRate) return pcm16;
    const ratio = fromRate / toRate;
    const srcSamples = pcm16.length / 2;
    const dstSamples = Math.floor(srcSamples / ratio);
    const out = Buffer.alloc(dstSamples * 2);
    for (let i = 0; i < dstSamples; i++) {
        const srcIdx = Math.floor(i * ratio) * 2;
        out.writeInt16LE(pcm16.readInt16LE(srcIdx), i * 2);
    }
    return out;
}

/** Return false when a PCM16 buffer contains a sample above the peak threshold. */
export function isSilentPcm16(buf, threshold = 800) {
    let peak = 0;
