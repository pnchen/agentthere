...
async function _getOpusEncoder() {
    ...
    return {
        encodeOpusRtp: _encodeOpusRtp,
        createOpusEncoder: _createOpusEncoder,
...
export class MediaOutSender {
    ...
    async _sendFrame(pcmFrame) {
        ...
        const { encodeOpusRtp, createOpusEncoder, FRAME_SAMPLES } = await _getOpusEncoder();
        if (!this._encoder) this._encoder = await createOpusEncoder();
        const rtp = await encodeOpusRtp(pcmFrame, this._seq, this._ts, this._ssrc, this._encoder);
        if (!rtp) return false;
        try {
