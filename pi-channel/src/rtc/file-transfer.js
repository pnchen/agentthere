...
import { getGroupPeers, getPeerByPeerId } from './index.js';
...
function waitForDrain(dc, timeoutMs = 30_000) {
    return new Promise(resolve => {
        ...
        const check = setInterval(() => {
            if (!dc.isOpen()) settle(false);
            else if (dc.bufferedAmount() < BUFFERED_AMOUNT_THRESHOLD) settle(true);
        }, 200);
    });
}
// ── chunk send ─────────────────────────────────────────────────────────
async function sendChunks(peer, objectId, fileBuffer, fileSize) {
    console.error(`[pi-channel/file] chunks start peer=${peer.peerId} object=${objectId} bytes=${fileSize}`);
    let offset = 0;
    while (offset < fileSize) {
        if (!peer.isConnected()) {
            console.error(`[pi-channel/file] ${peer.peerId} disconnected at offset ${offset}/${fileSize}`);
            return false;
        }
        if (!(await waitForDrain(peer.dc))) {
            console.error(`[pi-channel/file] ${peer.peerId} drain timeout at offset ${offset}/${fileSize}`);
            return false;
        }
        ...
        if (!peer.send(chunkMsg)) {
            console.error(`[pi-channel/file] ${peer.peerId} send failed at offset ${offset}/${fileSize}`);
            return false;
        }
        offset = end;
    }
    console.error(`[pi-channel/file] chunks complete peer=${peer.peerId} object=${objectId} bytes=${fileSize}`);
    return true;
}
// ── send to peers ──────────────────────────────────────────────────────
async function _sendToPeers({ filePath, fileName, mimeType, peerList, agentProfile, kind }) {
    console.error(
        `[pi-channel/file] begin path="${filePath}" file="${fileName}" mime=${mimeType} peers=${peerList.map(p => `${p.peerId}:connected=${p.connected}:dc=${!!p.dc}:open=${p.dc?.isOpen?.()}`).join(',')}`
    );
    ...
    for (const p of peerList) {
        ...
        console.error(`[pi-channel/file] metadata peer=${p.peerId} sent=${sent}`);
        if (!sent) return { ok: false, messageId, objectId };
    }
    console.error(`[pi-channel/file] metadata for "${fileName}" (${fileSize} bytes, ${peerList.length} peers)`);
    const fileBuffer = fs.readFileSync(filePath);
    const results = await Promise.allSettled(peerList.map(p => sendChunks(p, objectId, fileBuffer, fileSize)));
    const ok = results.filter(r => r.status === 'fulfilled' && r.value === true).length > 0;
    console.error(`[pi-channel/file] sent "${fileName}" — ${ok ? 'ok' : 'incomplete'}`);
    return { ok, messageId, objectId };
}
// ── public entry point ─────────────────────────────────────────────────
export async function sendMedia({ rawUrl, groupId, peerId, agentProfile, kind }) {
    console.error(`[pi-channel/file] sendMedia rawUrl="${rawUrl}" groupId=${groupId ?? ''} peerId=${peerId ?? ''}`);
    ...
    console.error(`[pi-channel/file] resolved filePath="${filePath}" fileName="${fileName}" mime=${mimeType}`);
    if (groupId) {
        const peerList = getGroupPeers(groupId);
        console.error(
            `[pi-channel/file] eligible peers group=${groupId} count=${peerList.length} ids=${peerList.map(p => p.peerId).join(',')}`
        );
