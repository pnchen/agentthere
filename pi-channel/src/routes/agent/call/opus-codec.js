 ...
 *
 * Adapted from openclaw-plugin/channel/src/channel/route/call/opus-codec.js
 * Kept verbatim; lazy-loads opus codec dependencies only when voice is used.
 *
...
export async function createOpusEncoder() {
    await _ensureOpus();
    if (!OpusScriptClass) return null;
    return new OpusScriptClass(48000, 1, OpusScriptClass.Application.AUDIO);
}

export async function encodeOpusRtp(pcm_buf, seq, ts, ssrc, encoder = null) {
    await _ensureOpus();
    const active_encoder = encoder ?? opus_encoder;
    if (!active_encoder) return null;
    let opus_payload;
    try {
        const _buf = Buffer.isBuffer(pcm_buf) ? pcm_buf : Buffer.from(pcm_buf);
        opus_payload = active_encoder.encode(_buf, FRAME_SAMPLES);
    }
    catch (e) {
